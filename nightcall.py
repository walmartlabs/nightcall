...
HTTP_FUZZ_LIST_REG = "/usr/share/wfuzz/wordlist/general/medium.txt"
HTTP_FUZZ_LIST_LRG = "/usr/share/wfuzz/wordlist/general/megabeast.txt"
...
NMAP_UDP = "nmap -v0 -n -Pn -O -sUV -p " + NMAP_UDP_PORTS + " --script \'" + NSE_SCRIPTS_UDP + \
   "\' --version-intensity 5 --max-retries 1 --max-rtt-timeout 300ms --max-scan-delay 300ms " \
...
DNS_SCAN = "dnsrecon {dnstcp}-a -n {host} -d $(host {hosttcp}{host} {host} | grep domain | tail -n1 " \
   "| rev | cut -d. -f2,3 | rev) > {host}.misc.dns"
...
# Metasploit Strings
MSF_STANDALONE = "msfconsole -q -x \"use {module}; set THREADS 6; setg RHOSTS {host}; " \
	"setg RHOST {host}; set RPORT {port}; set SSL {ssl}; run; exit\" -o {host}-{port}.msf.{ext}"
